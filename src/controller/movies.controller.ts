...
import movieClient from "../utils/movie.client";
...
export const getMoviesByYear = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const moviesResponse = await movieClient.get("/discover/movie", {
      params: {
        language: "en-US",
        page,
    ...
    const movies = await Promise.all(
      moviesResponse.data.results.map(async (movie: Movie) => {
        ...
        try {
          ...
          const creditsResponse = await movieClient.get(`/movie/${movie.id}/credits`);
